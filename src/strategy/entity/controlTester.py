...
from strategy.movements import goalkeep, spinGoalKeeper, goToBall, howFrontBall, howPerpBall, blockBallElipse
...
class ControlTester(Entity):
    ...
    def directionDecider(self):
       if self.robot.field is not None:
            ...
            if time.time()-self.lastChat > 0.7:
                if np.all(abs(angError(ref_th, rob_th)) > 90 * np.pi / 180):
    ...
    def fieldDecider(self):
        ...
        #Quad 1: X = +0.550 Y = +0.600
        #Quad 2: X = -0.550 Y = +0.600
        #Quad 3: X = -0.550 Y = -0.600
        ...
        if self.x == 2:
            if not -0.500 > rr[0] > -0.600 or not 0.550 < rr[1] < 0.650: #Não chegou no lugar certo
                self.robot.field = DirectionalField(ang(rr,[-0.550,0.600]), Pb=(-0.550,0.600, np.pi))
                return
            self.x = 3
        if self.x == 3:
            if not -0.500 > rr[0] > -0.600 or not -0.550 > rr[1] > -0.650: #Não chegou no lugar certo
        ...
        if self.x == 5:
            if not 0.500 < rr[0] < 0.600 or not 0.550 < rr[1] < 0.650: #Não chegou no lugar certo
                self.robot.field = AttractiveField(Pb=(0.550,0.600,np.pi))
                print(self.x)
                return
            self.x = 6
        if self.x == 6:
        ...
        if self.x == 9:
            if not 0.500 < rr[0] < 0.600 or not 0.550 < rr[1] < 0.650: #Não chegou no lugar certo
                self.robot.field = UVF((0.550,0.600,1.5*np.pi), direction=0) #AttractiveField(Pb=(-0.560,-0.600, np.pi)) 
                print(self.x)
                return
            self.x = 10
        if self.x == 10:
            if not -0.500 > rr[0] > -0.600 or not 0.550 < rr[1] < 0.650: #Não chegou no lugar certo
                self.robot.field = UVF((-0.550,0.600, np.pi), direction=0) #AttractiveField(Pb=(-0.560,-0.600, np.pi))
                print(self.x)
                return
            self.x = 11
        if self.x == 11:
            if not -0.500 > rr[0] > -0.600 or not -0.550 > rr[1] > -0.650: #Não chegou no lugar certo
                self.robot.field = UVF((-0.550,-0.600, 1,5*np.pi), direction=0)
                print(self.x)
                return
            self.x = 12
        if self.x == 12:
            if not 0.500 < rr[0] < 0.600 or not -0.550 > rr[1] > -0.650: #Não chegou no lugar certo
                self.robot.field = UVF((0.550,-0.600, np.pi), direction=0)
                print(self.x)
                return
            self.x = 9
